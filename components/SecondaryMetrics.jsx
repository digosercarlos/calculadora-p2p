...
export default function SecondaryMetrics({ results }) {
  ...
  const metrics = [
    ...
    },
    {
      ...
      color: results.esRentable ? 'text-emerald-400' : 'text-red-400',
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      color: results.spreadPct > 0 ? 'text-emerald-400' : 'text-red-400',
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {metrics.map((metric, index) => (
          ...
          >
            ...
            <div className={`${metric.color} text-lg font-mono font-semibold`}>
              {metric.value}
            </div>
          </div>
        ))}
